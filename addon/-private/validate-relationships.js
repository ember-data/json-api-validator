...
import {
  RELATIONSHIP_ERROR_TYPES,
  RelationshipError
} from "./errors/relationship-error";

export default function validateResourceRelationships(
  schema,
  relationships,
  methodName,
  path = "",
  ...
  for (let i = 0; i < foundRelationshipKeys.length; i++) {
    ...
    errors = errors.concat(validateResourceRelationship(schema, key, data, path, validator));
  }

  return errors;
}

function validateResourceRelationship(schema, propertyName, data, path, validator) {
  ...
  if (relationship === undefined) {
    errors.push(
      new RelationshipError(
        RELATIONSHIP_ERROR_TYPES.UNKNOWN_RELATIONSHIP,
        schema.type,
        propertyName,
        data,
        path
    ...
    if (typeof data !== "object") {
      errors.push(
        new Error(
          `The data for ${propertyName} on ${schema.type} is not an object`
        )
      );
    } else if (data !== null) {
      if (data.hasOwnProperty("links")) {
        if (typeof data.links !== "object") {
          errors.push(
            new Error(
              `The links for the relationship ${propertyName} on ${
                schema.type
      ...
      if (data.hasOwnProperty("meta")) {
        if (typeof data.meta !== "object") {
          errors.push(
            new Error(
              `The meta for the relationship ${propertyName} on ${
                schema.type
              } is not an object or null`
            )
          );
      ...
      if (data.hasOwnProperty("data")) {
        if (typeof data.data !== "object") {
          errors.push(
            new Error(
              `The data for the relationship ${propertyName} on ${
                schema.type
              } is not an object or null`
            )
          );
          ...
          if (relationship.kind === "hasMany") {
            if (!Array.isArray(data.data)) {
              errors.push(
                new Error(
                  `The data for the hasMany relationship ${propertyName} on ${
                    schema.type
                  } is not an array`
                )
              );
...
function validateRelationshipReference(relationship, ref, validator) {
  ...
  if (typeof ref.type !== "string" || ref.type.length === 0) {
    ...
    let schema = validator.schemaFor(type);
    ...
    if (!isCorrectType) {
      errors.push(
        new Error(`reference type is not valid for this relationship`)
      );
    }
  }
...
function _findRelationship(schema, propertyName, validator) {
  ...
  if (schema.inherits) {
    return _findRelationship(validator.schemaFor(schema.inherits), propertyName);
  }
